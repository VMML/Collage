...
#ifndef CO_CENTLOADAWAREDISTRIBUTOR_H
#define CO_CENTLOADAWAREDISTRIBUTOR_H
...
namespace co
{
namespace detail { class CentLoadAwareDistributor; }

class CentLoadAwareDistributor : public PackageDistributor
{
public:
    /** Construct a distributor that pushes packets equally (statically) to all clients. @version 1.x */
    CO_API CentLoadAwareDistributor( Producer& producer );

    /** Destruct this distributor. @version 1.x */
    virtual CO_API ~CentLoadAwareDistributor();
...
private:
    ...
    detail::CentLoadAwareDistributor* const _impl;
};

} // co

#endif // CO_CENTLOADAWAREDISTRIBUTOR_H
